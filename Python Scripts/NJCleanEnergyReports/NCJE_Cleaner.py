...
"""
THIS IS THE CLEANER THAT THE CLEAN ENERGY REPORT WEBSITE HANDS OUT, THIS IS USED TO
FIX THE NAMES OF THE COMPANIES, HOWEVER, THIS WILL NOT BE NEEDED IN THE FINAL CLEAN
ENERGY REPORT BECAUSR THE CODE CLEANS IT FIRST THERE, INSTEAD OF USING THIS
"""

class CLEAN_EXCEL():
    def __init__(self, Install_loc, Pipeline_loc):
        self.Install_loc = Install_loc
        self.Pipeline_loc = Pipeline_loc

    def Pipeline(self):
        # df = pd.read_excel('DATA+-+PIPELINE+-+August+2021.xlsx', sheet_name='TI - PIPELINE')
        # df = pd.read_excel(self.Pipeline_loc, sheet_name='TI - PIPELINE' )
        df = pd.read_excel(self.Pipeline_loc, sheet_name='TI Pipeline')
    ...
    def Full_Install(self):
        ...
        # df = pd.read_excel(self.Install_loc, sheet_name='TI - INSTALLED')
        df = pd.read_excel(self.Install_loc, sheet_name='TI - Installed')
        ...
        writer.save()
    # def run(self):
    #     Pipeline(self.Pipeline_loc)
    #     Full_Install(self.Install_loc)
